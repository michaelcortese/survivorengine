...
interface GameState {
    ...
    ) => { error: { content: string; ephemeral: boolean } } | { player: Player; targetPlayer?: Player };
}
...
const Game: GameState = {
    ...
    checkForError(interaction, hasTarget, requiredCard, interruptable, stoppingInteraction = false) {
        if (!Game.active) {
            return { error: { content: 'No game is currently in progress!', ephemeral: true } };
        }
        
        const player = Game.getPlayerFromUserId(interaction.user.id);
        if (!player) {
            return { error: { content: 'You are not a player in the current game!', ephemeral: true } };
        }
        
        let targetPlayer: Player | undefined;
        if (hasTarget) {
            const targetUserId = interaction.options.getUser('player')?.id || interaction.options.getUser('target')?.id;
            if (!targetUserId) {
                return { error: { content: 'You must specify a target player!', ephemeral: true } };
            }
            targetPlayer = Game.getPlayerFromUserId(targetUserId);
            if (!targetPlayer) {
                return { error: { content: 'The specified player is not in the game!', ephemeral: true } };
            }
        }
        
        
        if (interruptable && Game.interruption.active) {
            return { error: { content: 'This action cannot be played at this time. Wait a moment and try again.', ephemeral: true } };
        }
        
        if (stoppingInteraction && !Game.interruption.active) {
            return { error: { content: 'There is no interaction to stop!', ephemeral: true } };
        }

        // Remove card if they have it and error if not
    
        if (requiredCard && !player.hasCard(requiredCard)) {
            return { error: { content: `You must have the ${requiredCard} card to play this action!`, ephemeral: true } };
