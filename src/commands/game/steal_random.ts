...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!targetPlayer) {
      return interaction.reply({
        content: "You must specify a player to steal from.",
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    while (Game.interruption.active) {
      ...
      if (remaining !== lastDisplayedSecond) {
        ...
        await interaction.editReply({
          content: `Attempting to steal from <@${targetPlayer.id}>... (They have ~${remaining} seconds remaining to play "Sorry For You")`,
        });
      }
