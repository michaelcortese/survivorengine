import { SlashCommandBuilder, ChatInputCommandInteraction, MessageFlags, ActionRowBuilder, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle, ComponentType } from 'discord.js';
...
export default {
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        const result = Game.checkForError(interaction, HAS_TARGET, REQUIRED_CARD, INTERRUPTABLE, STOPPING_INTERACTION);
        if ('error' in result) {
            return interaction.reply({ content: result.error.content, flags: MessageFlags.Ephemeral });
        }
        const { player, targetPlayer } = result;
        if (!targetPlayer) {
            return interaction.reply({ content: 'You must specify a player to spy on.', flags: MessageFlags.Ephemeral });
        }
        if (targetPlayer.hand.length === 0) {
            //TODO give the card back to the player
            return interaction.reply({ content: `<@${targetPlayer.id}> has no cards to spy on!`, flags: MessageFlags.Ephemeral });
        }
        ...
        while (Game.interruption.active) {
            if (Game.interruption.stopped) {
                await interaction.editReply({ content: `Spy attempt was interrupted with ${lastDisplayedSecond} seconds remaining` });
                return interaction.followUp({ content: 'Your spy attempt was interrupted!', flags: MessageFlags.Ephemeral });
            }
            ...
            if (remaining !== lastDisplayedSecond) {
                ...
                await interaction.editReply({ content: `<@${player.id}> is attempting to spy on <@${targetPlayer.id}>... (They have ~${remaining} seconds remaining to play "Sorry For You")` });
            }

            await new Promise(resolve => setTimeout(resolve, 100)); // Check more frequently but update less
        }

        await interaction.editReply({ content: `<@${player.id}> is spying on <@${targetPlayer.id}>'s hand` });
        ...
        collector.on('collect', async (selectInteraction) => {
            if (selectInteraction.user.id !== interaction.user.id) {
                return selectInteraction.reply({ content: 'This is not your spy menu!', flags: MessageFlags.Ephemeral });
            }
            ...
            const updatedTakeButton = ButtonBuilder.from(takeButton).setDisabled(false);
            const updatedRow2 = new ActionRowBuilder<ButtonBuilder>().addComponents(updatedTakeButton, cancelButton);
        ...
        buttonCollector.on('collect', async (buttonInteraction) => {
            if (buttonInteraction.user.id !== interaction.user.id) {
                return buttonInteraction.reply({ content: 'This is not your spy menu!', flags: MessageFlags.Ephemeral });
            }
            ...
            if (selectedCardIndex === null) {
                return buttonInteraction.reply({ content: 'Please select a card first!', flags: MessageFlags.Ephemeral });
            }
            ...
            try {
                const targetUser = await interaction.client.users.fetch(targetPlayer.id);
                await targetUser.send(`<@${player.id}> spied on your hand and took **${cardToTake.getName()}** in the Survivor game!`);
            } catch (error) {
