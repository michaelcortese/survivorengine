...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!targetPlayer) {
            return interaction.reply({ content: 'You must specify a player to give a card to.', flags: MessageFlags.Ephemeral });
        }

        if (player.hand.length === 0) {
            return interaction.reply({ content: 'You have no cards to give!', flags: MessageFlags.Ephemeral });
        }
        ...
        collector.on('collect', async (selectInteraction) => {
            if (selectInteraction.user.id !== interaction.user.id) {
                return selectInteraction.reply({ content: 'This is not your give menu!', flags: MessageFlags.Ephemeral });
            }
            ...
            const updatedConfirmButton = ButtonBuilder.from(confirmButton).setDisabled(false);
            const updatedPrivateButton = ButtonBuilder.from(privateButton).setDisabled(false);
            const updatedRow2 = new ActionRowBuilder<ButtonBuilder>().addComponents(updatedConfirmButton, updatedPrivateButton);
        ...
        buttonCollector.on('collect', async (buttonInteraction) => {
            if (buttonInteraction.user.id !== interaction.user.id) {
                return buttonInteraction.reply({ content: 'This is not your give menu!', flags: MessageFlags.Ephemeral });
            }
            ...
            try {
                ...
                await targetUser.send(`You received **${cardToGive.getName()}** from <@${player.id}> in the Survivor game!`);
            } catch (error) {
